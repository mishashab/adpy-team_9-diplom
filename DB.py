...
# def find_ask_user(cur, user_id):
#     cur.execute('''
#         SELECT user_id FROM ask_user
#         WHERE user_id = %s;
#     ''', (user_id,))
#     return cur.fetchone()
...
#доработать случай когда блэк лист
def get_find_users(cur, user_id):
    cur.execute('''
        SELECT f_user_id, f_first_name, f_last_name, user_url, photo_str  FROM find_users
        JOIN photos ON find_users.f_user_id = photos.f_user_ids
        WHERE user_id = %s;
    ''', (user_id,))
    return cur.fetchall()
